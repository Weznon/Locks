...
public class LocksItems {
    public static final Item
            ...
            lock_pick = new ItemLockPickStrong("lock_pick"),
            lock_pick_2 = new ItemLockPickWeak("lock_pick_2"),
            key = new ItemKey("key"),
    ...
    public static void register(RegistryEvent.Register<Item> event) {
        event.getRegistry().registerAll(lock, lock_pick, key, master_key, key_blank, key_ring, lock_pick_2);
    }

    // TODO Don't create new mdl instances every time
    @SideOnly(Side.CLIENT)
    public static void registerModels(ModelRegistryEvent event) {
        registerModels(lock, lock_pick, lock_pick_2, key, master_key, key_blank);
        ItemMeshDefinition md = new ItemMeshDefinition() {
            @Override
            public ModelResourceLocation getModelLocation(ItemStack stack) {
                int keys = 0;
                IItemHandler inventory = LocksCapabilities.getInventory(stack);
                for (int a = 0; a < inventory.getSlots() && keys < 3; ++a)
                    if (!inventory.getStackInSlot(a).isEmpty()) ++keys;
